...
def get_args_parser():
    ...
    parser.add_argument('--epochs', default=820, type=int, help='Number of epochs of training.')
...
def train(args):
    ...
    # if the network is a XCiT
        ...
        teacher = torch.hub.load('facebookresearch/xcit:main',\
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # ============ training one epoch of ... ============
        train_stats = train_one_epoch(student, teacher,\
                teacher_without_ddp, aucm_loss,
...
def train_one_epoch(student, teacher, teacher_without_ddp, loss_fn, data_loader,
                    optimizer, lr_schedule, wd_schedule, momentum_schedule,epoch,
                    fp16_scaler, args):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Epoch: [{}/{}]'.format(epoch, args.epochs)
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader,\
            10, header)):
...
class Loss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        teacher_out = F.softmax((teacher_output - self.center) \
                / temp, dim=-1)

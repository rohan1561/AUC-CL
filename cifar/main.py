...
def train(net, data_loader, train_optimizer, loss_fn):
    net.train()
    total_loss, total_num, train_bar = 0.0, 0, tqdm(data_loader)
    for pos_1, pos_2, target in train_bar:
        pos_1, pos_2 = pos_1.to(DEVICE, non_blocking=True),\
        ...
        mask = (torch.ones_like(sim_matrix) - torch.eye(2*batch_size,\
                device=DEVICE)).bool()
        neg = torch.flatten(sim_matrix.masked_select(mask)) # [2*B, 2*B-1]
        ...
        pos = torch.cat([pos, pos], dim=0) # 2*B

        # compute loss
        pos_labels = torch.ones_like(pos)
        neg_labels = torch.zeros_like(neg)
...
if __name__ == '__main__':
    ...
    save_name_pre = f'lr{lr}bs{batch_size}'
